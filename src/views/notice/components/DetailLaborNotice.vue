<template>
    <div>
        <el-dialog v-model="detailLaborNoticeVisible" title="公告编辑" width="50%" before-close="">
            ...
            <template #footer>
                <span>
                    <el-button @click="cancelChangeLaborNotice(ruleFormRef)">取消</el-button>
                    <el-button type="primary" @click="changeLaborNotice(ruleFormRef)">
                        修改公告
                    </el-button>
                </span>
            </template>
...
import { updateLaborNotice } from '../../../api/laborNotice'

const detailLaborNoticeVisible = ref(false);
...
const deliverLaborNoticeForm = (laborNoticeInfo) => {
    ({
        noticeId:noticeForm.noticeId,
        title:noticeForm.title,
        collegeId:noticeForm.collegeId,
        termId:noticeForm.termId,
        advisor:noticeForm.advisor,
...
function cancelChangeLaborNotice(formEl: FormInstance | undefined){
    if(!formEl) return;
    detailLaborNoticeVisible.value = false;
    formEl.resetFields();
}

async function changeLaborNotice(formEl: FormInstance | undefined){
    if (!formEl) return
    await formEl.validate((valid, fields) => {
        if (valid) {
            updateLaborNotice(noticeForm).then(() => {
                ElMessage({
                    type: 'success',
                    message: '修改成功',
                })
...
defineExpose({
    detailLaborNoticeVisible,
    deliverLaborNoticeForm,
    termList,
    collegeList
})
