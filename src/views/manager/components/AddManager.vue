<template>
    <el-dialog v-model="addManagerVisble" title="添加管理员" width="25%">
        <el-form :model="managerForm" label-width="120px" inline :rules="rules" ref="ruleFormRef">
            ...
            <el-form-item label="用户密码" prop="userPassword">
                <el-input v-model="managerForm.userPassword" placeholder="请填写用户密码" class="w-55" />
            </el-form-item>
        ...
        <template #footer>
            <span>
                <el-button @click="CancelAddManager(ruleFormRef)">取消</el-button>
                <el-button type="primary" @click="AddManager(ruleFormRef)">
                    添加
                </el-button>
            </span>
        </template>
...
import { addManagerList, getManagerInfo } from '../../../api/manager';
import { CollegeList } from '../../laborPlan/type';

const addManagerVisble = ref(false);
...
async function AddManager(formEl: FormInstance | undefined) {
    if (!formEl)
        return;
    await formEl.validate((valid) => {
        if (valid) {
            addManagerList(managerForm).then(() => {
                ElMessage({
                    type: 'success',
                    message: '添加成功'
                });
                formEl.resetFields();
                emits('updateManager');
                addManagerVisble.value = false;
            })
        }
    })
}

function CancelAddManager(formEl: FormInstance | undefined) {
    if (!formEl)
        return;
    formEl.resetFields();
    addManagerVisble.value = false
